    ...
    <el-card>
      ...
      <div class="container_header">
        ...
        <div class="left">
          ...
          <!-- 个人信息部分 -->
          <div class="Info">
            <strong>工号：</strong>{{ teacherID }}<br />
            <strong>姓名：</strong>{{ teacherInfo.name }} <br />
            <strong>任教科目：</strong>{{ teacherInfo.subject }}
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getteacherInfo() {
      const myGet = (datatea) => {
        this.teacherInfo = datatea.data; //存储需要的数据，确保不丢失
      };
      this.item.id = window.sessionStorage.getItem("id");

      await fetch("http://175.27.240.116:5000/api/Teacher/GetById", {
        method: "POST",
        ...
        .then((datatea) => myGet(datatea)) //datastu储存转化完成的数据
    ...
    async gettableData() {
      ...
      await fetch("http://175.27.240.116:5000/api/Announce/GetList", {
        ...
        .then((response) => response.json()) //将获取的数据转化成json格式
        .then((tablelist) => myGet(tablelist)) //datal储存转化完成的数据
        .catch((error) => console.error("Unable to get items".error)); //报错信息
